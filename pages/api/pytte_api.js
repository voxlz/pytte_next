...
handler.get(async (req, res) => {
    ...
    if(date){
        doc = await req.db.collection(collection).findOne({date: new Date(date)})
    } else {
        doc = await req.db.collection(collection).findOne()
    }
    if(doc == null){
        doc = dataModel
    }
...
handler.post(async (req, res) => {
    ...
    let doc = await req.db.collection(collection).updateOne({date: new Date(data.date)}, {$set:data}, {upsert: true})
    res.json({message: 'ok'});
})
